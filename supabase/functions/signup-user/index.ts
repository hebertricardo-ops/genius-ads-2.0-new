...
serve(async (req) => {
  ...
  try {
    ...
    // Cria usuário via admin API — não dispara e-mail automático do Supabase
    const { data: userData, error: createError } = await supabaseAdmin.auth.admin.createUser({
      email,
      password,
      user_metadata: { name: name?.trim() ?? "", whatsapp: whatsapp ?? "" },
      email_confirm: false,
    });

    if (createError) {
      console.error("signup-user: createUser error:", createError.message);
    ...
    if (linkError) {
      console.error("signup-user: generateLink error:", linkError.message);
      // Usuário foi criado mas o e-mail não pôde ser enviado — retorna sucesso parcial
    ...
    if (!emailResponse.ok) {
      ...
      console.error(`signup-user: Resend error ${emailResponse.status}: ${errText.slice(0, 300)}`);
    } else {
      console.log(`signup-user: confirmation email sent to ${email}`);
    }

    return new Response(JSON.stringify({ success: true, emailSent: emailResponse.ok }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
